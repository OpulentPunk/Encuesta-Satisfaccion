...
def login():
    ...
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("""
            SELECT id_usuario, nombre, verificado, privilegios, contraseña, codigo_verificacion
            FROM usuarios 
            WHERE correo = %s
        """, (correo,))
        ...
        if usuario:
            ...
            if usuario[2] == 0 and usuario[5]:  # verificado=0 y tiene código de verificación
            ...
            if check_password_hash(usuario[4], contrasena):
                if usuario[2] == 1:  # verificado
                    ...
                    if usuario[3] == 1:  # Específicamente si es administrador (privilegio = 1)
